...
class EntryControllerTest extends TestCase
{
    ...
    public function test_handles_boolean_field_correctly(): void
    {
        $data = [
            'title' => 'Boolean Test',
            'slug' => 'boolean-test',
        ...
        $response = $this->postJson("/api/entries/{$this->contentType->slug}", $data);

        $response->assertStatus(201)
                 ->assertJsonFragment(['is_published' => true]);

        $this->assertDatabaseHas('entry_fields', [
            'value' => 1
        ]);
    }
}
